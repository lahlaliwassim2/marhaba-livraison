...
const register = async (req,res) => {
    const {body} = req
    if(!body.name || !body.email || !body.password ||!body.phone ) throw Error('Fill all filled')
    else{
        ...
        if(finduser) throw Error('email a already existed')
            ...
            const user = await User.create({...body, password: password_hash, role_id: client_role._id, status: 'Pending'})
...
const login = async (req ,res) => {
    ...
    const findUser = await User.findOne({email:body.email})
    if(!findUser || !(await bcrypt.compare(body.password, findUser.password))) throw Error('email or password is incorrect')
    if(findUser.status == 'Pending') throw Error('check your email')
    if(findUser.status == 'Banned') throw Error('you cant to login')
    const token = await jwt.sign({_id:findUser._id},process.env.SECRET_TOCKEN)
    storage('token', token)
...
const verifyEmail = async (req,res) => {
        ...
        const verify_email = await User.updateOne({email : find_user.email } , { $set:{status:'Active'}})
...
const resetPassword = async(req,res) => {
    const {body} = req
    if(!body.last_password || !body.new_password || body.new_password !=body.confirm_new_password)throw Error('Fill all filled')
    ...
    const verfy_last_password = await bcrypt.compare(body.last_password, find_user_reset.password)
...
const veriyfyForgetPassword = async (req,res) => {
 const token = req.params.token
    ...
    const new_token = await jwt.sign({id:verify_token_email._id},process.env.SECRET_TOCKEN)
...
const formeForgetPassword = async (req,res) => {
    const {body} = req
    if(!body.password ||body.password != body.confirm_password) throw Error('password not required')
    const verfy_form_tocken = await jwt.verify(storage('new_token'),process.env.SECRET_TOCKEN)
    const find_forget_password = await User.findById(verfy_form_tocken.id)
    if(!find_forget_password) throw Error('Error not found,replay to check your email')
    const hash_forget_password = await bcrypt.hash(body.password,10)
    await User.updateOne({_id:find_forget_password._id},{$set:{password:hash_forget_password}})
    res.json({message : 'your password is changed'})
